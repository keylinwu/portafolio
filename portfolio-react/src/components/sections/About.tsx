...
export function About() {
  ...
  const sectionTitle = vibe === 'interesting' ? 'A little about me' : 'About Me'
          ...
          <AnimatedSection delay={0.1}>
            <motion.div
              className={`relative overflow-hidden ${
                vibe === 'interesting' ? 'rounded-3xl rotate-[-2deg]' : 'rounded-2xl'
              }`}
              whileHover={
                vibe === 'interesting' ? { rotate: 0, scale: 1.03 } : { scale: 1.02 }
              }
              transition={{ type: 'spring', stiffness: 200 }}
            >
              <img
                src={`${import.meta.env.BASE_URL}images/${vibe === 'interesting' ? 'anime-cr.png' : 'anime-2.png'}`}
